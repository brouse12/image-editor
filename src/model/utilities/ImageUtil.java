...
public class ImageUtil {
  ...
  public static void writeImage(int[][][] rgb, int width, int height, String filename)
    ...
    BufferedImage output = arrayToImage(rgb, width, height);
    String extension = filename.substring(filename.lastIndexOf(".") + 1);
    ImageIO.write(output, extension, new FileOutputStream(filename));
  }

  /**
   * Convert a 3D array of image pixels to a BufferedImage. Useful for writing images to files or
   ...
   * @param width  the width of the image.
   * @param height the height of the image.
   * @return a BufferedImage version of the array.
   */
  public static BufferedImage arrayToImage(int[][][] rgb, int width, int height) {
